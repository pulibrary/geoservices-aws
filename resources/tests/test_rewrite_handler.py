...
def test_rewrites_mosaic_id_production_id_in_url():
    event = {'Records': [{'cf': {'request': {'uri': 'https://test.com/banana/mosaicjson'}}}]}
...
def test_rewrites_mosaic_id_staging_id_url():
    event = {'Records': [{'cf': {'request': {'uri': 'https://test.com/banana/mosaicjson'}}}]}
...
def test_rewrites_cog_production_id_in_url():
    ...
    with mock.patch('urllib3.PoolManager', return_value=http_mock):
        event = {'Records': [{'cf': {'request': {'uri': 'https://test.com/banana/cog'}}}]}
        output = handler_production(event, {})
...
def test_rewrites_cog_staging_id_in_url():
