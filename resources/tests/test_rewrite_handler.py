...
def test_rewrites_mosaic_id_production():
    http_mock = TestHelper().http_mock("s3://figgy-geo-production/ba/na/na/banana/mosaic-34.json")
...
def test_rewrites_mosaic_id_staging():
    http_mock = TestHelper().http_mock("s3://figgy-geo-staging/ba/na/na/banana/mosaic-34.json")
    with mock.patch('urllib3.PoolManager', return_value=http_mock):
        event = {'Records': [{'cf': {'request': {'uri': 'https://test.com/mosaicjson', 'querystring': 'id=banana'}}}]}
        output = handler_staging(event, {})
